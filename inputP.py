...
# get_shoe_size, getProdType, getUserInfo, createProfile, getAutoCheckout, getWatchOption
...
def get_auto_checkout(profile_dict):
    ...
    if auto_checkout == 'y':
        ...
        get_auto_checkout(profile_dict)
...
def get_new_prod(prod_dict):
    ...
    new_prod = input('Would you like to enter another product? (y/n): ')
...
def get_new_profile(profile_dict):

    new_profile = input('\nWould you like to create another profile? (y/n): ')

    if new_profile == 'y':
        # create_profile(profile_dict, None, None)
        return True
